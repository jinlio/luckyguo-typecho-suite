...
**身份与文案**
- 站点名称、作者名称与标识、副标题，以及首页 SEO 标题和描述。首页 SEO 字段留空时回退到站点名称和副标题。
...
**站点**
- 站点开始运行时间（用于运行时长展示）。
- 文章页会输出 `BlogPosting` JSON-LD、作者、封面、发布时间、更新时间和 BreadcrumbList。搜索结果页自动使用 `noindex,follow`；分页页面使用自身 canonical。若要给单篇文章指定搜索摘要，可在 Typecho 自定义字段中填写 `seoDescription` 或 `metaDescription`。
