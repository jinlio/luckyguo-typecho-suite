...
3. 打开主题设置页，按顺序填写：站点名称、作者名称与标识、副标题、首页眉题与签名、文章列表标题、文章作者和目录标签、个人简介、头像地址（可选）、首页横幅（可选）、文章封面（可选）、个人主页与代码仓库链接、强调色、主题 Cookie 名称与域名、默认主题模式、页面与正文宽度、阅读速度、站点起始时间，以及搜索入口、阅读进度、代码增强、页面动画、访问统计、首页附加模块、文章目录、评论 RSS、阅读信息、Gravatar 等开关。每个字段都有占位说明，不确定的可以留空。
...
**身份与文案**
- 站点名称、作者名称与标识、副标题。
- 关于页：引导语、方向、技术栈、状态、模块标题、模块副标题、详细简介。设置详细简介后，该内容优先于关于页面的正文。
- 首页眉题、签名、签名说明、文章列表标题与英文标签。
- 文章作者标签、文章目录标签。

**资料、链接与视觉**
- 个人简介（多行文本）。
- 头像地址、首页横幅、文章封面、个人主页、代码仓库。每个字段接受 HTTP(S) 地址；留空时显示中性主题标识或隐藏对应区域。
...
- 评论头像 Gravatar 开关，默认关闭；关闭时使用主题内置本地默认头像，访客邮箱哈希不会离开服务器。
