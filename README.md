...
The settings page controls site name, author name and handle, tagline, biography, homepage eyebrow/signature/list labels, article author and TOC labels, the About page stack cards/current direction/writing direction/contact email, avatar, comment avatar source, Gravatar endpoint, homepage banner, site sharing cover (1200x630), favicon, article cover, the article-top cover display switch, article-level `thumbnail`/`cover`/`image` custom fields, homepage/code links, preset or custom accent color, cookie name/domain, default light/dark mode, page and reading widths, reading speed, uptime start time, statistics switch, homepage auxiliary modules, tag-cloud or recent-comment widget, table-of-contents depth, reading metadata, reading progress, search entry, code enhancements, page motion, comment RSS, excerpt length, archive limit, and counter bucket count. About stack rows use `name|icon|description`; `icon` may be a short text mark or an HTTPS brand icon URL. The home page hides zero-count categories and includes a configurable tag cloud by default. Article pages include copy-link, X, Weibo, Telegram, and QR sharing controls.
...
Leave the cookie domain empty for a host-only preference. Set it only when trusted subdomains need to share it. Image fields accept HTTP(S) URLs; the article-top cover switch is off by default, while article cover URLs remain available for social sharing and can be enabled later from the theme settings. A missing author avatar shows a neutral theme mark, while a missing banner or article cover is omitted. Comment avatars are local by default, so visitor email hashes are not sent to an external service unless `useGravatar` is enabled. When Gravatar is enabled, the endpoint can be changed to a reachable mirror such as `https://gravatar.loli.net/avatar/`; only the lowercase email hash is sent. Statistics are disabled by default and require `deploy/create-suite-stats.sql`.
...
All of these controls are in Typecho Admin -> Plugins -> SuiteMonitor -> Settings. The first three fields, Monitor brand name/handle/avatar URL, are the personal identity shown on the monitor page. The navigation textarea controls Console, Home, Landing, and custom links; the footer checkbox controls whether a repository link is shown. Empty brand fields inherit the active theme's site name, author handle, and avatar. The About block title, subtitle, and long introduction are configured at Appearance -> Settings -> About page module title, About page module subtitle, and About page detailed introduction; the configured introduction takes precedence over the page body.
