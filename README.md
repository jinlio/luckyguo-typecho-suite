...
3. Open the theme settings page and fill in: site name, author name and handle, tagline, homepage eyebrow / signature, post list title, article author and TOC labels, personal bio, avatar URL (optional), banner URL (optional), article cover URL (optional), landing and code repository links, accent colour, theme cookie name and domain, default theme mode, page and reading widths, reading speed, site start time, the toggles for search, reading progress, code enhancements, motion, statistics, home widgets, article TOC, comments RSS, reading metadata, and Gravatar. Each field has placeholder text; leave anything you do not understand empty.
...
- About page: leading sentence, focus area, stack, status, module title, module subtitle, and the long detailed introduction. The detailed introduction takes precedence over the page body when set.
...
- Avatar URL, banner URL, article cover URL, landing URL, code repository URL. Each accepts an HTTP(S) URL; empty values fall back to the neutral theme mark or hide the slot.
...
- Gravatar switch for comment avatars. Off by default; comment avatars are rendered from the bundled local default SVG so visitor email hashes never leave your server.
