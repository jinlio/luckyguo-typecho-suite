...
final class RebuildStore
{
    ...
    {
        $sql = 'SELECT * FROM ' . $this->table('suite_searchmeta') . ' WHERE id = 1'
        ...
        if (!$row) {
            throw new \RuntimeException('Missing suite_searchmeta singleton row');
        }
    ...
    public function markBuild(string $batchId, int $startEpoch): void
    {
        $this->shortTransaction(function () use ($batchId, $startEpoch): void {
            ...
            $statement = $this->prepare(
                'UPDATE ' . $this->table('suite_searchmeta')
    ...
    public function finishSuccess(string $version, int $documentCount): void
    {
        ...
        $statement = $this->prepare(
            'UPDATE ' . $this->table('suite_changequeue')
    ...
    {
        $statement = $this->prepare(
            'SELECT DISTINCT cid FROM ' . $this->table('suite_changequeue')
    ...
    {
        $statement = $this->prepare(
            'SELECT id, cid FROM ' . $this->table('suite_changequeue')
    ...
    {
        $statement = $this->prepare(
            'SELECT COUNT(*) AS total FROM ' . $this->table('suite_changequeue')
    ...
    public function prepareTask(string $batchId, string $operation, string $indexUid): int
    {
        $statement = $this->ledgerConnection->prepare(
            'INSERT INTO ' . $this->table('suite_rebuildtask')
    ...
    {
        $statement = $this->ledgerConnection->prepare(
            'UPDATE ' . $this->table('suite_rebuildtask')
    ...
    {
        $statement = $this->ledgerConnection->prepare(
            'SELECT * FROM ' . $this->table('suite_rebuildtask')
