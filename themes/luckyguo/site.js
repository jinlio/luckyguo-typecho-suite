(() => {
  ...
  const saveTheme = (theme) => {
    localStorage.setItem('luckyguo-theme', theme);
    document.cookie = `luckyguo-theme=${theme}; Max-Age=31536000; Path=/; Domain=.luckyguo.dpdns.org; SameSite=Lax; Secure`;
  };
  ...
  const updateScrollState = () => {
    ...
    const start = article.getBoundingClientRect().top + scrollY - (header?.offsetHeight ?? 0);
    const distance = Math.max(article.offsetHeight - innerHeight * .55, 1);
  ...
  if (!motionQuery.matches && 'IntersectionObserver' in window) {
    ...
    const revealTargets = document.querySelectorAll([
      ...
      '.article-content > *', '.article-aside', '.about-masthead > *',
